 ...
 * Top Albums Widget
 */
class Widget_Top_Albums extends WP_Widget {

	/*
	 * Instantiate
	 */
	function Widget_Top_Albums() {
		$widget_ops = array(
			'description' => __( 'Last.fm Top Albums' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct(
			'topalbums',
			__( 'Top Albums', 'lastfm-utils' ),
			$widget_ops );
	}
	...
	function widget( $args, $instance ) {
		$title = empty( $instance['title'] ) ? __( 'Top Albums', 'lastfm-utils' ) : $instance['title'];
		...
		$this->output_topalbums( $this->id, $limit, $period, $color, $link_target );
	 ...
	 * アルバムをリストで出力
	 */
	private function output_topalbums( $widget_id, $limit, $period, $color, $link_target ) {
		?>
		<div class="message <?php echo $color; ?>"><div class="loader"></div><span>Loading...</span></div>
		<ul class="list <?php echo $color; ?>"></ul>
		<script>jQuery(document).ready(function () {
			load_lastfmdata('<?php echo $widget_id; ?>', 'topalbums', '<?php echo $link_target; ?>', '<?php echo $limit; ?>', '<?php echo $period; ?>');
		});
		</script>
		<?php
	}
