 ...
 * Top Tracks Widget
 */
class Widget_Top_Tracks extends WP_Widget {

	/*
	 * Instantiate
	 */
	function Widget_Top_Tracks() {
		$widget_ops = array(
			'description' => __( 'Last.fm Top Tracks' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct(
			'toptracks',
			__( 'Top Tracks', 'lastfm-utils' ),
			$widget_ops );
	}
	...
	function widget( $args, $instance ) {
		$title = empty( $instance['title'] ) ? __( 'Top Tracks', 'lastfm-utils' ) : $instance['title'];
		...
		$this->output_toptracks( $this->id, $limit, $period, $color, $link_target );
	 ...
	 * トラックをリストで出力
	 */
	private function output_toptracks( $widget_id, $limit, $period, $color, $link_target ) {
		?>
		<div class="message <?php echo $color; ?>"><div class="loader"></div><span>Loading...</span></div>
		<ul class="list <?php echo $color; ?>"></ul>
		<script>jQuery(document).ready(function () {
			load_lastfmdata('<?php echo $widget_id; ?>', 'toptracks', '<?php echo $link_target; ?>', '<?php echo $limit; ?>', '<?php echo $period; ?>');
		});
		</script>
		<?php
	}
